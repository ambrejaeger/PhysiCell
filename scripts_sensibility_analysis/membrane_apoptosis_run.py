...
from hill_response import read_value_in_settings_file
...
if val:
    ...
    print(f"Error: Could not read value in settings file.")
...
def evaluate_epi_growth_2(
    ...
    for i in range(end_file - start_file):
        ...
        with open(output_storage_file, "a") as f:
            f.write(
                f"{i + start_file} {output_apoptosis_nbr[i]} {output_apoptosis_pos[i]} {output_epi_sizes[i]}\n"
            )
