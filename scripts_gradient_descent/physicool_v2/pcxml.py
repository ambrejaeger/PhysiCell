...
ConfigFileParser (from the config module) instead. To safely write to the CSV cell rule 
file, use the CellRuleFileParser (from the config module) instead.
"""
...
def parse_death_model(
    tree: ElementTree, path: str, name: str
) -> Dict[str, Union[str, float, List[float], None]]:
    """
...
def parse_ruleset(file_path: Union[str, Path]) -> List[List[str]]:
    ruleset = []
...
def parse_ruleset_params(tree: ElementTree, path: str) -> Dict[str, Union[str, Union[str, Path]]]:

    if tree.find(path).tag != "ruleset":
        raise ValueError("The passed path does not point to the correct node.")

    folder = Path(tree.find(path + "/folder").text)
    filename = str(tree.find(path + "/filename").text)
...
def write_ruleset_params(
    new_values: Dict[str, Union[str, Path]], tree: ElementTree, path: str
) -> None:
    if tree.find(path).tag != "ruleset":
        raise ValueError("The passed path does not point to the correct node.")

    tree.find(path + "/folder").text = str(new_values["folder"])
    tree.find(path + "/filename").text = str(new_values["filename"])

def write_cell_rule(line_index: int, new_rule: Dict[str, Union[str, float, bool]], ruleset: List[List[str]]) -> None:
    keys = [
