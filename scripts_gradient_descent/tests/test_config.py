...
class PhysiCellConfigTest(unittest.TestCase):
    def setUp(self):
        ...
        copyfile(RULES_PATH, RULES_PATH_WRITE)
        self.xml_data = config.ConfigFileParser(CONFIG_PATH)
        self.xml_write = config.ConfigFileParser(WRITE_PATH)
        self.rules_read = config.CellRuleFileParser(RULES_PATH)
        self.rules_write = config.CellRuleFileParser(RULES_PATH_WRITE)
    ...
    def test_read_cell_rules(self):
        expected_data = [
            dt.Rule(**EXPECTED_RULESET_READ[0]), 
            dt.Rule(**EXPECTED_RULESET_READ[1]), 
            dt.Rule(**EXPECTED_RULESET_READ[2])
            ]
        expected_data = dt.RuleSet(rules=expected_data)
        data = self.rules_read.read_ruleset()
        self.assertEqual(expected_data.rules, data.rules)
    ...
    def test_write_cell_rule(self):
        """Asserts that the cell rule data is properly written."""
        data = self.rules_write.read_ruleset()
        rule = data[1]
        print("Original rule: ", rule) #the rule is okay here
        rule.base_value = 0.0
        rule.half_max = 0.5
        rule.hill_power = 1.0
        rule.apply_to_dead = True
        self.rules_write.write_cell_rules(1, rule)

        rule = self.rules_write.read_ruleset()[1]

        print("expected: ", EXPECTED_CELL_RULE_WRITE
              )
        print("new rule: ", rule)
        self.assertEqual(EXPECTED_CELL_RULE_WRITE, rule.model_dump())
